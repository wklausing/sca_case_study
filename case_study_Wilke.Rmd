...
ggplot(growth_by_month[growth_by_year$period != 1,], aes(x=period, y=monthly_growth, group=1))+
  geom_line(color="red") +
  ggtitle(label = "Wachstum von Total Revenue pro Monat")
...
*Decompose TS*
```{r}
birthsComp <- decompose(ts_shipments)
birthsComp
...
shipments_complete_dates %>% as_tbl_time(.)
