...
```{r}
# Umsatz, Kosten, Marge (international)
...
### (2.6) 
#### (2.6.1) Grafiken zu Cancellation Reasons
```{r}
...
ggplot(subset(shipments_cancellation_reason, cancellation_reason != "" & cancellation_reason != "customer_not_ready" & cancellation_reason != "other"& cancellation_reason != "self_cancelled" & first_stop_requested_arrival_to_YearMonth <= "2020-12"))+
  ...
  theme_classic()
```
#### (2.6.2) Moeglicher Zusammenhang zwischen Stornierungsgrund und anderen Faktoren 
```{r}
# Erschaffen eines subsets von shipments nur mit stornierten Aufträgen von 2018-2020
shipments_cancelled = subset(shipments, cancellation_reason != "" & 
                       first_stop_requested_arrival_to_YearMonth >= "2018-01" & 
                       first_stop_requested_arrival_to_YearMonth <= "2020-12" )
```

a. Faktor: business_model (3PL, 4PL)
```{r}
#Stornierungsgründe nach business_model
shipments_cancelled_business_model = shipments_cancelled[, c("YearMonth", "business_model","cancellation_reason")]

df_business_model = data.frame(xtabs(~ cancellation_reason + business_model, data = shipments_cancelled_business_model))
df_business_model_reshaped= reshape(df_business_model,
                            timevar="business_model",
                            idvar = c("cancellation_reason"),
                            direction = "wide")
df_business_model_reshaped
```
```{r}
# Prozentualer Anteil der stonierten Antraegen bei 3PL Geschaeftsmodell
( count((subset(shipments_cancelled, business_model == "3PL")))/ count((subset(shipments, business_model == "3PL"))))*100  

# Prozentualer Anteil der stonierten Antraegen bei 4PL Geschaeftsmodell
( count((subset(shipments_cancelled, business_model == "4PL")))/ count((subset(shipments, business_model == "4PL"))))*100 
```
Erkenntnisse:
- bei 3PL und 4PL: der Stornierungsgrund customer_not_ready tritt am meisten auf
- Es werden 7,28% der Antraege bei 3PL und 4,28% bei 4PL storniert.

b. Faktor: fulfillment_strategy (bulk, pallet, fullTruck)
```{r}
#Stornierungsgründe nach Fulfillment-Strategie
shipments_cancelled_fulfillment_strategy = shipments_cancelled[, c("YearMonth", "fulfilment_strategy","cancellation_reason")]

df_fulfillment_strategy = data.frame(xtabs(~ cancellation_reason + fulfilment_strategy , data = shipments_cancelled_fulfillment_strategy))
df_fulfillment_strategy_reshaped= reshape(df_fulfillment_strategy,
                            timevar="fulfilment_strategy",
                            idvar = c("cancellation_reason"),
                            direction = "wide")
df_fulfillment_strategy_reshaped
```

```{r}
# Prozentualer Anteil der stonierten Antraegen bei Fulfillment-Strategie bulk
( count((subset(shipments_cancelled, fulfilment_strategy == "bulk")))/ count((subset(shipments, fulfilment_strategy == "bulk"))))*100  
# Prozentualer Anteil der stonierten Antraegen bei bei Fulfillment-Strategie fullTruck
( count((subset(shipments_cancelled, fulfilment_strategy == "fullTruck")))/ count((subset(shipments, fulfilment_strategy == "fullTruck"))))*100  
# Prozentualer Anteil der stonierten Antraegen bei bei Fulfillment-Strategie pallet
( count((subset(shipments_cancelled, fulfilment_strategy == "pallet")))/ count((subset(shipments, fulfilment_strategy == "pallet"))))*100 
```
Erkenntnisse:
- der am meisten auftretende Stornierungsgrund bei der Fulfillment-Strategy:
bulk: self_cancelled
fullTruck: customer_not_ready
pallet: self_cancelled
- Prozentualer Anteil der stonierten Antraegen bei Fulfillment-Strategie:
bulk: 4,78%
fullTRuck: 8,72%
pallet: 3,1%

c. Faktor: stops_count
```{r}
#Stornierungsgründe nach Stoppanzahl
shipments_cancelled_Stoppanzahl = shipments_cancelled[, c("YearMonth", "stops_count","cancellation_reason")]

df_Stoppanzahl = data.frame(xtabs(~ cancellation_reason + stops_count , data = shipments_cancelled_Stoppanzahl))
df_Stoppanzahl_reshaped= reshape(df_Stoppanzahl,
                            timevar="stops_count",
                            idvar = c("cancellation_reason"),
                            direction = "wide")
df_Stoppanzahl_reshaped
```
```{r}
# Prozentualer Anteil der stonierten Antraegen bei Stoppanzahl von 2
( count((subset(shipments_cancelled, stops_count == 2)))/ count((subset(shipments, stops_count == 2))))*100  

# Prozentualer Anteil der stonierten Antraegen bei bei Stoppanzahl von 3
( count((subset(shipments_cancelled, stops_count == 3)))/ count((subset(shipments, stops_count == 3))))*100  

```
c. Faktor: lane_domain
```{r}
#Stornierungsgründe nach Lanedomain
shipments_cancelled_lane_domain = shipments_cancelled[, c("YearMonth", "lane_domain","cancellation_reason")]

df_lane_domain = data.frame(xtabs(~ cancellation_reason + lane_domain , data = shipments_cancelled_lane_domain))
df_lane_domain_reshaped= reshape(df_lane_domain,
                            timevar="lane_domain",
                            idvar = c("cancellation_reason"),
                            direction = "wide")
df_lane_domain_reshaped
```
```{r}
# Prozentualer Anteil der stonierten Antraegen bei lane domain export
( count((subset(shipments_cancelled, lane_domain == "export")))/ count((subset(shipments, lane_domain == "export"))))*100  

# Prozentualer Anteil der stonierten Antraegen bei lane domain import
( count((subset(shipments_cancelled, lane_domain == "import")))/ count((subset(shipments, lane_domain == "import"))))*100  

# Prozentualer Anteil der stonierten Antraegen bei lane domain international
( count((subset(shipments_cancelled, lane_domain == "international")))/ count((subset(shipments, lane_domain == "international"))))*100  

# Prozentualer Anteil der stonierten Antraegen bei lane domain national
( count((subset(shipments_cancelled, lane_domain == "national")))/ count((subset(shipments, lane_domain == "national"))))*100  


```
- Prozentualer Anteil der stonierten Antraegen bei lane domain:
export: 5,48%
import: 10,55%
international: 7,38%
national: 6,86%
...
plot_ly(data = margin,x= ~first_stop_requested_arrival_to_week,   
        y= ~cancel_rate_3pl ,type = "bar", color = I("darkred"))%>% 
        layout(title = 'Cancellation rate in 3PL Shipments (Europaweit)', xaxis = list(title = 'Monat', dtick= 10)) 
...
```{r}
# durchschnittliche Marge, aggregiert nach Geschaefstmodell (neu)

# subset seit der Einfuehrung des neuen Geschaeftsmodell 4PL (ab 08.11.2019)
shipments_uncancelled_ab_2019_11 = subset(shipments_uncancelled, first_stop_requested_arrival_to_date >= "2019-11-08")

# durchschnittliche Marge, aggregiert nach Geschaefstmodell
shipments_uncancelled_ab_2019_11 %>%
    group_by(business_model) %>%
    summarize(mean_margin = mean(total_margin, na.rm = TRUE))

# Visualisierung
ggplot(shipments_uncancelled_ab_2019_11, 
       aes(factor(business_model), 
       fill = factor(business_model))) +
      geom_bar()
...
### (3.3) OTD
```{r}
# durchschnittliche Marge, aggregiert nach OTD
  shipments_uncancelled %>%
    group_by(OTD) %>%
    summarize(mean_margin = mean(total_margin, na.rm = TRUE))
...
shipments_uncancelled_ohne_NA = subset(shipments_uncancelled, OTD == "TRUE" | OTD == "FALSE")

ggplot(shipments_uncancelled_ohne_NA, 
       aes(factor(OTD), 
       fill = factor(OTD))) +
       geom_bar()
```
...
```{r}
model_business_model = lm(total_margin ~business_model, data = shipments_uncancelled )
summary(model_business_model)

model_custom_rate_shipment = lm(total_margin ~custom_rate_shipment, data = shipments_uncancelled )
summary(model_custom_rate_shipment)

model_fulfillment_strategy = lm(total_margin ~fulfilment_strategy, data = shipments_uncancelled )
summary(model_fulfillment_strategy)

model_lane_domain = lm(total_margin ~lane_domain, data = shipments_uncancelled )
summary(model_lane_domain)

model_stops_count = lm(total_margin ~stops_count, data = shipments_uncancelled )
summary(model_stops_count)

model_OTD = lm(total_margin ~OTD, data = shipments_uncancelled )
summary(model_OTD)
...
### (4) Deskriptive Statistik (Zeitreihenanalyse)
