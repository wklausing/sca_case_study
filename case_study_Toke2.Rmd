...
shipments_clean_info
shipments_clean_dates
...
```{r}
subset(shipments, cancellation_reason =="") %>%
  count(OTD)
```


## Prüfen der Länder Codes
```{r}
...
```
# Anzahl der Stops pro uncancalled Auftrag
```{r}
subset(shipments_clean_info, cancellation_reason == "") %>%
  count(stops_count)

ggplot(data = subset(shipments_clean_info, cancellation_reason == "") %>%
  count(stops_count))+
  geom_col(aes(stops_count,n))+
  ylab("Anzahl Aufträge")+
  xlab("Anzahl Stops")+
  theme_classic()+
  ggtitle("Anzahl Aufträge nach Anzahl stops")
```

```{r}
custom_rate_Nr = shipments_clean_info %>%
  count(custom_rate_shipment)
        
  colnames(custom_rate_Nr)[2]= "Gesamtanzahl_Aufträge"

storno_Nr = subset(shipments_clean_info, cancellation_reason != "") %>%
  count(custom_rate_shipment)

colnames(storno_Nr)[2]= "Stornierte_Aufträge"

ggplot(data = custom_rate_Nr %>%
  inner_join(storno_Nr, c("custom_rate_shipment")) %>%
  mutate(Anteil_Storno = Stornierte_Aufträge / Gesamtanzahl_Aufträge))+
  geom_col(aes(custom_rate_shipment,Anteil_Storno, fill = c("blue","red")))+
  theme_classic()+
  labs(
    title = "Anteil Stornierte Aufträge",
    y = "Anteil stornierte Aufträge"
  )+
  theme(legend.position = "none")
```
Erkenntniss: Aufträge im Rahmen con Kontrakten werden mehr als doppelt so häufig storniert als Spot Aufträge

```{r}
shipments_clean_info %>%
  count(lane_domain)

subset(shipments_clean_info, lane_domain == "international") %>%
  count(origin_country)
```

##Korrelation mit eigenem Dataframe prüfen
```{r}
...
```{r}
shipments %>%
  count(first_stop_requested_arrival_to_YearMonth)
