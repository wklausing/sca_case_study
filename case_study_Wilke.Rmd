...
# Korrelationen suchen
  #Shipments
  shipments2 = readRDS(file = "data/shipments.rds")
  shipments2 = shipments2[shipments2$year >= 2018 & shipments2$year <= 2020 & shipments2$lane_domain == "national",]
  shipments2 = shipments2[shipments2$total_margin > 0 & shipments2$total_revenue > 0 & shipments2$total_cost > 0,]
  ...
  #Diesel
  diesel <- read.csv("data/dieselPreiseMonat.csv", sep=";")
  shipments_cor = merge(shipments_cor,diesel[c("Dieselkraftstoff","YearMonth")],by=c("YearMonth","YearMonth"))
  ...
  shipments_cor = merge(shipments_cor,int_Trade_Deutschland,by=c("YearMonth","YearMonth"),all.x = TRUE)
  
  
  shipments_cor$YearMonth = NULL
  shipments_correlations_total_margin = cor(shipments_cor,shipments_cor[c("total_margin")]) %>% as.data.frame()
