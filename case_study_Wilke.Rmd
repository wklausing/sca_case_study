...
#Baseline erstellen, durschnittliche Marge vom Monat.
  ...
  baseline = merge(baseline,shipments[c("month","first_stop_agreed_arrival_to")],by=c("month","month"))

  shipments_externals_month$YearMonth = shipments$first_stop_agreed_arrival_to %>% as.Date() %>% strftime(., format = "%Y-%m")
  
  shipments_externals_month = merge(baseline,shipments[c("month","first_stop_agreed_arrival_to")],by=c("month","month"))
  
  shipments_externals_month$YearMonth = substr(shipments_externals_month$YearMonth,1,nchar(shipments_externals_month$YearMonth)-3)

  shipments_externals_month = merge(shipments_externals_month,baseline[c("month","total_margin")],by=c("month","month"))
  
  shipments_externals_month %>% str()
...
#Baseline Bewertung aufstellen
  ...
  # MAE berechnen
  evaluation[evaluation$Model == "Baseline",]$MAE = (shipments_externals_month$total_margin.x - shipments_externals_month$total_margin.y) %>% abs() %>% mean()
  
  # sMAPE berechnen
  evaluation[evaluation$Model == "Baseline",]$sMAPE = smape(shipments_externals_month$total_margin.x, shipments_externals_month$total_margin.y)
...
#Modell erstellen
  m1_1 = shipments_monthsTraining %>% lm(total_margin.x ~ AvgHealth, .)
```
```{r}
...
#Baseline erstellen, durschnittliche Marge vom Monat.
  ...
  shipments_externals_month = merge(shipments_externals_month,baseline[c("month","total_margin_baseline")],by=c("month","month"))
...
#Modell erstellen, 1. Iteration
  m1_1 = shipments_monthsTraining %>% lm(total_margin.x ~ Dieselkraftstoff, .)
  m1_2 = shipments_monthsTraining %>% lm(total_margin.x ~ Spedition...Deutschland., .)
  m1_3 = shipments_monthsTraining %>% lm(total_margin.x ~ Transport...Deutschland., .)
...
#Modell erstellen, 2. Iteration
  m1_2_1 = shipments_monthsTraining %>% lm(total_margin.x ~ Dieselkraftstoff, .)
  m1_2_3 = shipments_monthsTraining %>% lm(total_margin.x ~ Spedition...Deutschland., .)
