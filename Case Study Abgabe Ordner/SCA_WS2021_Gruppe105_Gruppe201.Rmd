...
#6. Entferne ersten 4 Zeilen, siehe Forum: System als Demo vermutlich verwendet. Betrifft 4 Zeilen.
...
### (1.5) Marge in Abhängigkeit mit Weekday (mit Subsets erstellen)
```{r}
...
# subset shipments_uncancelled mit 2 neuen Spalten weekday_first_stop und weekday_last_stop
...
# (2.1) Liniendiagram zu Margin der gesamten 36 Perioden (2018-2020)
```{r}
...
ggplot(marge_YearMonth, aes(x = seq(1,length(c(marge_YearMonth$first_stop_requested_arrival_to_YearMonth)),1), y = total_margin, group = 1)) + 
  ...
  labs(x = "Periode",
       ...
       title =  "Entwicklung der durchschnittlichen Marge (2018-2020)")+theme_standard
  
```

### (2.2) Shipments Gegenueberstellung: total_cost und total_revenue
```{r eval=FALSE}
# Umsatz aggrerieren
...
### (2.3) Auswertung der Fullfilment-Strategie über die Kontrakt und Spot Aufträge
```{r}
...
### (2.4) Abhängigkeit der Marge von Kontrakt und Spot Aufträgen
```{r}
...
### (2.5) Abhängigkeit der Marge von der Lane Domain
```{r}
#Anzahl Aufträge nach Lane Domain
ggplot(data = shipments_uncancelled %>%
...
# Durchschnittliche Marge nach Lane Domain
...
```{r}
# subset erstellen: origin_count, distination_count und country_count
...
# Visualisierung von origin_count nach Ländern
...
# Visualisierung von destination_count nach Ländern
ggplot(data =subset(shipments %>%
...
# Visualisierung für Anzahl der Aufträge von Top 10 Auftragsrouten
...
### (2.10) Liniendiagramme zu Shipment Kosten, Umsatz und Marge
```{r eval=TRUE}
...
```{r}
#  Visualisierung von Umsatz, Kosten und Marge (Europaweit)
...
# Visualisierung von Umsatz, Kosten und Marge(Deutschland)
...
# Tabelle ausgeben
shipments_YearMonth_aggregated_national
...
```{r eval=TRUE}
# Visualisierung nur von der Marge (Europaweit)
...
shipments_YearMonth_aggregated

```

```{r eval=TRUE}
# Visualisierung nur von der Marge (Deutschland)
...
### (2.11) Ein Liniendiagramm zu Shipment transport_price_booked + transport_cost_assigned
```{r eval=TRUE}
# Aggregieren
...
# Liniendiagramm Uebersicht für Buchungspreis, Transportkosten und Differenz (2017-2021)
ggplot(data = shipments_week_aggregated, aes(x = YearMonth, group=1)) + 
  ...
  xlab("Date") +  
  ylab("Euro") +
  ggtitle(label = "Instafreight: Buchungspreis, Transportkosten und Differenz (2017-2020)") +
  theme_standard+
...
# Liniendiagramm Differenz zu Buchungs- und Transportkoste (2017-2021)
ggplot(data = shipments_week_aggregated, aes(x = YearMonth, group=1)) + 
  geom_line(aes(y = difference), colour="black") + 
  xlab("Date") +  
  ylab("Euro") +
  ggtitle(label = "Instafreight: Differenz zu Buchungs- und Transportkoste (2018-2021)") +
  theme_standard+
  theme(axis.text.x = element_text(angle=45, vjust = 1.1, hjust=1.2, size=8)) + 
  #y-Achse Zahlen als Integer-Wert deklarieren
  scale_y_continuous(labels = scales::number_format(accuracy = 100)) +
...
### (2.12) Grafiken zu Stornierungsgründen
```{r}
...
### (2.13) Shipments Gegenuberstellung: Lieferungskosten gebucht und zugewiesen
```{r eval=TRUE}
...
### (2.14) Visualisierung der zeitlichen Entwicklung von dem prognostizierten (prozentualen) wöchentlichen Profit
```{r}
# Vorhandene Vorhersage aus Margin visualisieren
...
```{r}
# Absolute Differenz zwischen forcats revenue und actual revenue
...
ggplot(data = margin) +
 geom_line(aes(x = first_stop_requested_arrival_to_week, y = differenz_fcast_actual, group = 1), color = "darkgreen")+
  ...
  xlab("Week") +  
  ylab("Euro") +
  ggtitle(label = "Differenz zwischen prognostizierten und realisierten Margen (2018-2021)") +
  theme(axis.text.x = element_text(angle=45, vjust = 1.1, hjust=1.2, size=8)) + 
  scale_x_discrete(breaks = margin$first_stop_requested_arrival_to_week[seq(1, length(margin$first_stop_requested_arrival_to_week), by = 10)])
...
### (2.15) Visualisierung der zeitlichen Veränderung des Anteils von Stornierungsrate (3PL Shipment) 
```{r}
...
### (2.16) Visualisierung der prognostizierten Profite aus custom rate shipments und spot shipments
...
### (3) Zusammenhang zwischen der Marge und internen Faktoren 
...
### (3.2) Qualitativer Zusammenhang zwischen der Gewinnmarge und nicht numerischen internen Faktoren: business model, custom rate shipment, fulfillment strategy und lane domain 
Qualitativer Zusammenhang zwischen der Gewinnmarge (numerical) und internen Faktoren: business model, custom rate shipment, fulfillment strategy und lane domain (categorical)
...
# Häufigkeitstabelle von custom_rate_shipment und fulfillment_strategy
...
### (3.3) Zusammenhang zwischen der Gewinnmarge und internem Faktor: stop count
...
### (3.4) Vorbereitung für den Teil B
```{r}
...
####(4.1.1) Visualiesierung Vorhersage Total Marge
...
#### (4.2) Zeitreihenanalyse Total durchschnittliche Marge 
```{r eval=TRUE}
...
#### (4.2.1) Vorhersage Total durchschnittliche Marge 
Erstellen einer Nachfragevorhersage mit der 'forcast' Funktion und dem Modell 'm_marge_3PL' für ein weiteres Jahr also 12 Perioden
```{r}
#Vorhersage erstellen
...
fcast_marge
```
#### (4.2.2) Visualiesierung Vorhersage Total durchschnittliche Marge 
Visualiesierung der Vorhersage ("forecast") und der ursprünglichen Nachfragen ("original") in einem Liniendiagramm über die Perioden. Es wurde ein Liniendiagramm gewählt, da es den fließenden Verlauf zwischen den Perioden anzeigen lässt und dadurch wie eng die beiden Linien der Vorhersage und der Origanlwerte über die Zeit auseinander liegen, lässt sich die Güte des Modells beobachten
```{r}
# Hintergrund erzeugen 
ggplot(data = NULL) + 
  ...
  geom_smooth(data = NULL, aes(x=seq(1,length(c(fcast_marge$fitted, fcast_marge$mean)),1), 
                             ...
                             colour = 'forecast'))+
    #Diagramm Titel hinzufügen
  ggtitle(label = "Ø Marge (2018-2020)")+
  # Umbenennung der x- und y-Achsen
...
#### (4.3) Zeitreihenanalyse Prozentuale Marge 
```{r}
...
#### (4.3.1) Vorhersage Prozentuale Marge 
Erstellen einer Nachfragevorhersage mit der 'forcast' Funktion und dem Modell 'm_marge_3PL' für ein weiteres Jahr also 12 Perioden
```{r}
#Vorhersage erstellen
...
#### (4.3.2) Visualisierung Prozentuale Marge 
Visualiesierung der Vorhersage ("forecast") und der ursprünglichen Nachfragen ("original") in einem Liniendiagramm über die Perioden. Es wurde ein Liniendiagramm gewählt, da es den fließenden Verlauf zwischen den Perioden anzeigen lässt und dadurch wie eng die beiden Linien der Vorhersage und der Origanlwerte über die Zeit auseinander liegen, lässt sich die Güte des Modells beobachten
```{r}
# Hintergrund erzeugen 
ggplot(data = NULL) + 
...
#### (4.4) Zeitreihenanalyse Aufträge
```{r}
...
#### (4.4.2) Vorhersage AuftragslageVisualisierung Vorhersage
```{r}
# Hintergrund erzeugen 
ggplot(data = NULL) + 
...
#### (4.5) Zeitreihenanalyse fulfimemt Strategie
```{r}
...
#### (4.5.1) Vorhersage Auftragslage bei verschiedenen Fulfillment-Strategien
```{r}
#Vorhersage erstellen
...
#### (4.5.2) Visualisierung Vorhersage Auftragslage bei verschiedenen Fulfillment-Strategien
```{r}
...
#### (4.6) Zeitreihenanalyse custome_rate
```{r}
custom_rate_count_YearMonth = shipments_uncancelled_national %>% count(first_stop_requested_arrival_to_YearMonth, custom_rate_shipment)
```

```{r}
custom_rate_count_YearMonth_reshaped= reshape(custom_rate_count_YearMonth,
                         timevar="custom_rate_shipment",
                         idvar = c("first_stop_requested_arrival_to_YearMonth"),
                         direction = "wide")
colnames(custom_rate_count_YearMonth_reshaped)=c("YearMonth","Einzelaufträge", "Kontraktaufträge")
head(custom_rate_count_YearMonth_reshaped)
```

```{r eval=TRUE}
ts_Einzelaufträge = ts(custom_rate_count_YearMonth_reshaped$Einzelaufträge, frequency = 12)
ts_Kontraktaufträge = ts(custom_rate_count_YearMonth_reshaped$Kontraktaufträge, frequency = 12)
...
# Modell automatisch erstellen lassen
cat("\n","\n","Modell:","\n")
m_Einzelaufträge = ets(ts_Einzelaufträge, model = "ZAA")
m_Kontraktaufträge = ets(ts_Kontraktaufträge, model = "ZAA")
...
# (3) Residuen ausgeben
cat("\n","\n","Residuen:","\n")
round(m_Einzelaufträge$residuals,2)
round(m_Kontraktaufträge$residuals,2)

```

#### (4.6.1) Vorhersage Auftragslage 
```{r}
#Vorhersage erstellen
fcast_Einzelaufträge = forecast(m_Einzelaufträge,12)
fcast_Kontraktaufträge = forecast(m_Kontraktaufträge,12)

#Anzeigen der Vorhersage
fcast_Einzelaufträge
fcast_Kontraktaufträge
```

#### (4.6.2) Visualisierung Vorhersage
```{r}
...
ggplot(data = NULL) + 
  ...
  # Erste Linie hinzufuegen (keine Daten; X-Achse = Reihe von 1 bis laenge fitting Werten und Forecast Werten)
  geom_line(data = NULL, aes(x=seq(1,length(c(fcast_Einzelaufträge$fitted, fcast_Einzelaufträge$mean)),1), 
                             # Y-Achse = Kombination von fitting Werten und Forecast Werten
                             y= c(as.numeric(fcast_Einzelaufträge$fitted),as.numeric(fcast_Einzelaufträge$mean)), 
                             # Beschriftung fuer Farbe festlegen
                             colour = 'forecast')) +
  
  # Zweite Linie hinzufuegen (Keine Daten; X-Achse = Reihe von 1 bis laenge Beobachtungen)
  geom_line(data = NULL, aes(x=seq(1,length(fcast_Einzelaufträge$x),1), 
                             # Y-Achse = Beobachtungen
                             y= as.numeric(fcast_Einzelaufträge$x),
                             # Beschriftung fuer Farbe festlegen
                             colour = 'original')) + 
  ...
  ## Vorhersage Kontraktaufträge
  # Hintergrund erzeugen 
ggplot(data = NULL) + 
  
  # Erste Linie hinzufuegen (keine Daten; X-Achse = Reihe von 1 bis laenge fitting Werten und Forecast Werten)
  geom_line(data = NULL, aes(x=seq(1,length(c(fcast_Kontraktaufträge$fitted, fcast_Kontraktaufträge$mean)),1), 
                             # Y-Achse = Kombination von fitting Werten und Forecast Werten
                             y= c(as.numeric(fcast_Kontraktaufträge$fitted),as.numeric(fcast_Kontraktaufträge$mean)), 
                             # Beschriftung fuer Farbe festlegen
                             colour = 'forecast')) +
  
  # Zweite Linie hinzufuegen (Keine Daten; X-Achse = Reihe von 1 bis laenge Beobachtungen)
  geom_line(data = NULL, aes(x=seq(1,length(fcast_Kontraktaufträge$x),1), 
                             # Y-Achse = Beobachtungen
                             y= as.numeric(fcast_Kontraktaufträge$x),
                             # Beschriftung fuer Farbe festlegen
                             colour = 'original')) + 
  ...
  #TODO Periode in Jahr/Monat umwandeln
```

### (4.6.2) Visualisierung Vorhersage
```{r}
## Vorhersage Einzelaufträge
Kontrakte_CalenderWeek = subset(shipments %>%
  count(CalenderWeek,custom_rate_shipment), custom_rate_shipment == "true") %>%
  select(-custom_rate_shipment)
...
# shipments_week
  shipments_week = shipments[c("CalenderWeek","stops_count","transport_price_booked","transport_cost_assigned","total_revenue","total_cost","total_margin","OTD")] %>% aggregate(. ~ CalenderWeek, ., mean)
...
# Google Trends
  ...
  google_data$CalenderWeek = google_data$Woche %>% as.Date() %>% strftime(., format = "%Y-%V")# Wochen Spalte
  google_data = google_data[c("CalenderWeek","Spedition...Deutschland.", "Transport...Deutschland.")] %>% aggregate(. ~ CalenderWeek , ., mean)
...
# DAX
  ...
  DAX$CalenderWeek = DAX$Datum %>% as.Date() %>% strftime(., format = "%Y-%V")# Jahr-Monat Spalte
  DAX = DAX[c("CalenderWeek","daxDayMean")] %>% aggregate(. ~ CalenderWeek , ., mean)
  shipments_week = merge(shipments_week,DAX,by=c("CalenderWeek","CalenderWeek"),all.x = TRUE)
...
#### (4.7) Zeitreihenanalyse durchschnittliche transport Kosten assigned
```{r eval=TRUE}
cost_YearMonth_mean =  aggregate(transport_cost_assigned ~ YearMonth, shipments_uncancelled_national, mean)

ts_cost_mean = ts(cost_YearMonth_mean$transport_cost_assigned, frequency = 12)

#Die 'cat' Funktion dient lediglich für eine bessere Übersicht indem sie Absätze im Output erzeugt
# Modell automatisch erstellen lassen
cat("\n","\n","Modell:","\n")
m_cost_mean = ets(ts_cost_mean, model = "ZAA")
# (1) Modellzusammenfassung ausgeben lassen
m_cost_mean
...
# (3) Residuen ausgeben
cat("\n","\n","Residuen:","\n")
round(m_cost_mean$residuals,2)
```


#### (4.7.1) Vorhersage für durchschnittliche transport Kosten assigned
Erstellen einer Nachfragevorhersage mit der 'forcast' Funktion und dem Modell 'm_marge_3PL' für ein weiteres Jahr also 12 Perioden
```{r}
#Vorhersage erstellen
fcast_cost_mean = forecast(m_cost_mean, 12)
#Anzeigen der Vorhersage
fcast_cost_mean
```


#### (4.7.2) Vorhersage für durchschnittliche transport Kosten assigned
Visualiesierung der Vorhersage ("forecast") und der ursprünglichen Nachfragen ("original") in einem Liniendiagramm über die Perioden. Es wurde ein Liniendiagramm gewählt, da es den fließenden Verlauf zwischen den Perioden anzeigen lässt und dadurch wie eng die beiden Linien der Vorhersage und der Origanlwerte über die Zeit auseinander liegen, lässt sich die Güte des Modells beobachten
```{r}
# Hintergrund erzeugen 
ggplot(data = NULL) + 
  ...
  # Erste Linie hinzufuegen (keine Daten; X-Achse = Reihe von 1 bis laenge fitting Werten und Forecast Werten)
  geom_line(data = NULL, aes(x=seq(1,length(c(fcast_cost_mean$fitted, fcast_cost_mean$mean)),1), 
                             # Y-Achse = Kombination von fitting Werten und Forecast Werten
                             y= c(as.numeric(fcast_cost_mean$fitted),as.numeric(fcast_cost_mean$mean)), 
                             # Beschriftung fuer Farbe festlegen
                             colour = 'forecast')) +
  
  # Zweite Linie hinzufuegen (Keine Daten; X-Achse = Reihe von 1 bis laenge Beobachtungen)
  geom_line(data = NULL, aes(x=seq(1,length(fcast_cost_mean$x),1), 
                             # Y-Achse = Beobachtungen
                             y= as.numeric(fcast_cost_mean$x),
                             # Beschriftung fuer Farbe festlegen
                             colour = 'original')) + 
  geom_smooth(data = NULL, aes(x=seq(1,length(c(fcast_cost_mean$fitted, fcast_cost_mean$mean)),1), 
                             # Y-Achse = Kombination von fitting Werten und Forecast Werten
                             y= c(as.numeric(fcast_cost_mean$fitted),as.numeric(fcast_cost_mean$mean)), 
                             # Beschriftung fuer Farbe festlegen
                             colour = 'forecast'))+
    #Diagramm Titel hinzufügen
  ggtitle(label = "Ø Transportkosten assigned (2018-2020)")+
  # Umbenennung der x- und y-Achsen
  labs(y="Ø Kosten in EUR", x="Periode")+
  theme_classic()+
  theme(
      axis.text = element_text( color = "grey30", size = 12),
    legend.text = element_text( color = "grey30", size = 12),
