...
#TODO
#13. Falsche Ländercodes entfernt.
shipments <- shipments %>%
  separate(country_lane, c("origin_country","destination_country"), "-->>")
...
shipments$shipment_created_at_date = shipments$first_stop_agreed_arrival_to %>% as.Date()
shipments_dates = shipments[shipments$first_stop_agreed_arrival_to >= "2018-01-01" & shipments$first_stop_agreed_arrival_to <= "2020-12-31",]
shipments_dates = shipments_dates[c("first_stop_agreed_arrival_to","total_margin")]

alldates = seq(as.Date("2018-01-01"), as.Date("2020-12-31"), by="days")#Erzeuge alle Daten von 2018 bis 2020
dates0 = alldates[!(alldates %in% shipments_dates$first_stop_agreed_arrival_to)]#Vektor mit alle Daten die shipments_dates fehlen.
dates_and_margin = data.frame(first_stop_agreed_arrival_to = dates0, total_margin = 0)#DF mit fehlenden Daten und erzeuge total_margin = 0
shipments_complete_dates = rbind(shipments_dates, dates_and_margin)#Verbinde DFs
shipments_complete_dates = shipments_complete_dates[order(shipments_complete_dates$first_stop_agreed_arrival_to),]#Sortiere

shipments_complete_dates = shipments_complete_dates %>% aggregate(total_margin ~ first_stop_agreed_arrival_to, ., mean) %>% as_tibble(.)
...
plot(x=decomposed_shipments$first_stop_agreed_arrival_to,y=decomposed_shipments$remainder)

# Jahr-Monat Spalte
decomposed_shipments$YearMonth = decomposed_shipments$first_stop_agreed_arrival_to %>% as.Date() %>% strftime(., format = "%Y-%m") 
...
ggplot(shipments_complete_dates, aes(x = agreed, y = total_margin, group = 1)) + 
  geom_line() +
...
*Regression: Korrelationen suchen*
```{r}
...
# Korrelationen suchen
  #Shipments
  shipments_externals_month = readRDS(file = "data/shipments.rds")
  shipments_externals_month = shipments_externals_month[shipments_externals_month$year >= 2018 & shipments_externals_month$year <= 2020 & shipments_externals_month$lane_domain == "national" & shipments_externals_month$total_revenue > 0 & shipments_externals_month$total_cost > 0,]
  shipments_externals_month = shipments_externals_month[c("YearMonth","stops_count","transport_price_booked","transport_cost_assigned","total_revenue","total_cost","total_margin","OTD")] %>% aggregate(. ~ YearMonth, ., mean)
  
  #Google Trends
  google_data = read.csv("data/google_data.csv")
  google_data$YearMonth = google_data$Woche %>% as.Date() %>% strftime(., format = "%Y-%m")# Jahr-Monat Spalte
  google_data = google_data[c("YearMonth","Spedition...Deutschland.", "Transport...Deutschland.")] %>% aggregate(. ~ YearMonth , ., mean)
  shipments_externals_month = merge(shipments_externals_month,google_data,by=c("YearMonth","YearMonth"))
  ...
  shipments_externals_month = merge(shipments_externals_month,Lkw_Maut_Fahrleistungsindex,by=c("YearMonth","YearMonth"))
  ...
  shipments_externals_month = merge(shipments_externals_month,DAX,by=c("YearMonth","YearMonth"),all.x = TRUE)
  
  #International Trade_ Deutschland_2018-2020.xlsx
  int_Trade_Deutschland = read_excel("data/International Trade_ Deutschland_2018-2020.xlsx", sheet = 1)
  names(int_Trade_Deutschland)[1] <- "YearMonth"
  shipments_externals_month = merge(shipments_externals_month,int_Trade_Deutschland,by=c("YearMonth","YearMonth"),all.x = TRUE)
  
  
  shipments_correlations_total_margin = shipments_externals_month
  shipments_correlations_total_margin$YearMonth = NULL
  shipments_correlations_total_margin = cor(shipments_correlations_total_margin,shipments_correlations_total_margin[c("total_margin")]) %>% as.data.frame()
  shipments_correlations_total_margin
  
  shipments_correlations = shipments_cor %>% cor()
  shipments_correlations
  
  #Wir nehmen Spedition...Deutschland., Lkw.Maut.Fahrleistungsindex, Trade Value (Million Euro), total_cost, transport_cost_assigned, daxDayMean, und OTD
  shipments_externals_month = shipments_externals_month[c("total_margin","Spedition...Deutschland.", "Lkw.Maut.Fahrleistungsindex", "Trade_Value_Million_Euro", "total_cost", "transport_cost_assigned", "daxDayMean", "OTD")]
  shipments_externals_month
  shipments_externals_month %>% cor()
...
evaluation_function <- function(evaluation, model) {
          ...
          # DF erweitern.
          evaluation = rbind(evaluation, data.frame(Model = 
                               modelname, MAE = numeric(1), sMAPE = numeric(1), r  = as.numeric(1), r2  = as.numeric(1))) %>% round(4)
          ...
          # R berechnen.TODO Richtig Berechnung einfügen
          evaluation[evaluation$Model == modelname,]$r = summary(model)$r.squared %>% round(4)
          
          # R**2 berechnen.TODO Richtig Berechnung einfügen
          evaluation[evaluation$Model == modelname,]$r2 = summary(model)$adj.r.squared %>% round(4)
...
#Baseline erstellen, durschnittliche Marge vom Monat.
  shipments = readRDS(file = "data/shipments.rds")
  shipments = shipments[shipments$year >= 2018 & shipments$year <= 2020 & shipments$lane_domain == "national" & shipments$total_revenue > 0 & shipments$total_cost > 0,]
  baseline = shipments %>% aggregate(total_margin ~ month , ., mean)
  names(baseline)[2] = "total_margin_baseline"
  baseline_3y = rbind(baseline,baseline)
  baseline_3y = rbind(baseline_3y,baseline)
...
#Modell für Baseline
  baseline_3y
  m_baseline = shipments_monthsTraining %>% lm(total_margin ~ Spedition...Deutschland., .)
...
#Baseline Bewertung aufstellen
  ...
  # MAE berechnen
  evaluation[evaluation$Model == "Baseline",]$MAE = (shipments_externals_month$total_margin - baseline_3y$total_margin) %>% abs() %>% mean() %>% round(4)
  
  # sMAPE berechnen 
  evaluation[evaluation$Model == "Baseline",]$sMAPE = smape(shipments_externals_month$total_margin, baseline_3y$total_margin_baseline) %>% round(4)
  
  # R berechnen # R berechnen.TODO Richtig Berechnung einfügen
  evaluation[evaluation$Model == "Baseline",]$r = smape(shipments_externals_month$total_margin, baseline_3y$total_margin_baseline) %>% round(4)
  
  # R**2 berechnen # R**2 berechnen.TODO Richtig Berechnung einfügen
  evaluation[evaluation$Model == "Baseline",]$r2 = smape(shipments_externals_month$total_margin, baseline_3y$total_margin_baseline) %>% round(4)
...
*1. Iteration*
```{r}
#Spedition...Deutschland.	-0.272131997	
  #Lkw.Maut.Fahrleistungsindex	-0.597840957	
  #Trade Value (Million Euro)	-0.503885420
  #total_cost	-0.390081398	
  #transport_cost_assigned	-0.359119956	
  #OTD	0.339293886
#Modell erstellen, 1. Iteration
  m_Google_spedition = shipments_monthsTraining %>% lm(total_margin ~ Spedition...Deutschland., .)
  m_LKWMaut = shipments_monthsTraining %>% lm(total_margin ~ Lkw.Maut.Fahrleistungsindex, .)
  m_TradeValue = shipments_monthsTraining %>% lm(total_margin ~ Trade_Value_Million_Euro, .)
  m_total_cost = shipments_monthsTraining %>% lm(total_margin ~ total_cost, .)
  m_cost_assigned = shipments_monthsTraining %>% lm(total_margin ~ transport_cost_assigned, .)
  m_dax = shipments_monthsTraining %>% lm(total_margin ~ daxDayMean, .)
  m_otd = shipments_monthsTraining %>% lm(total_margin ~ OTD, .)
  
  evaluation = evaluation_function(evaluation, m_Google_spedition)
  evaluation = evaluation_function(evaluation, m_LKWMaut)
  evaluation = evaluation_function(evaluation, m_TradeValue)
  evaluation = evaluation_function(evaluation, m_total_cost)
  evaluation = evaluation_function(evaluation, m_cost_assigned)
  evaluation = evaluation_function(evaluation, m_dax)
  evaluation = evaluation_function(evaluation, m_otd)
  
  evaluation
  
  summary(m_Google_spedition)
  summary(m_LKWMaut)
  summary(m_TradeValue)
  summary(m_total_cost)
  summary(m_cost_assigned)
  summary(m_dax)
  summary(m_otd)
  
  
# m2 hat niedrigsten MAE mit 12.47933, 
