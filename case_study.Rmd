...
plot_ly(data = shipments_week_aggregated,x= ~YearMonth,   
        ...
        #Hinzufügen von Regressionslinie
        add_lines(x=~YearMonth, y=predict(lm(margin~total_revenue, data = shipments_week_aggregated)), line=list(dash="solid", width = 1.5, color=NULL), name="Trend_Margin")%>%
...
# Alternative mit plot_ly
plot_ly(data = shipments_week_aggregated,x= ~YearMonth,   
        y= ~margin,type = "scatter", mode = "lines", name = "Total Cost") %>%  
        #Hinzufügen von Regressionslinie
        add_lines(x=~YearMonth, y=predict(lm(margin~total_revenue, data = shipments_week_aggregated)), line=list(dash="solid", width = 1.5, color=NULL), name="Trend_Margin")%>%
